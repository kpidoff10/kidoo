...
class BLEManagerClass {
  ...
  /**
   * Écrire un tag NFC
   * Fonction utilitaire qui simplifie l'appel d'écriture NFC
   */
  async writeNFCTag(
    blockNumber: number,
    data: number[],
  ...
  /**
   * Lire un tag NFC
   * Fonction utilitaire qui simplifie l'appel de lecture NFC
   */
  async readNFCTag(
    blockNumber: number,
    options?: {
   ...
   * @returns Réponse avec les informations système (firmwareVersion, model, etc.)
   */
  async getSystemInfo(options?: {
    timeout?: number;
    timeoutErrorMessage?: string;
  }): Promise<SystemInfoResponse> {
    const commandJson = JSON.stringify({ command: 'VERSION' });
    const success = await this.sendCommand(commandJson);
    
    if (!success) {
      throw new Error('Erreur lors de l\'envoi de la commande VERSION');
    }

    // Attendre la réponse avec message "VERSION"
    const response = await this.waitForResponse({
      expectedMessage: 'VERSION',
      timeout: options?.timeout,
      timeoutErrorMessage: options?.timeoutErrorMessage || 'Timeout: aucune réponse reçue pour VERSION',
    });

    // Vérifier que c'est bien une réponse de succès avec les champs requis
    if (response.status === 'success' && response.message === 'VERSION' && response.firmwareVersion && response.model) {
      return response as SystemInfoResponse;
    }

    throw new Error('Réponse invalide pour VERSION');
  }
   ...
   * Commande globale commune à tous les modèles
   * @param options Options de timeout (optionnel)
   * @returns Réponse avec la luminosité en pourcentage (10-100) dans brightness
   */
  async getBrightness(options?: {
    ...
    if (!success) {
      throw new Error('Erreur lors de l\'envoi de la commande GET_BRIGHTNESS');
    }

    // Attendre la réponse avec message "BRIGHTNESS_GET"
    const response = await this.waitForResponse({
      expectedMessage: 'BRIGHTNESS_GET',
      timeout: options?.timeout,
      timeoutErrorMessage: options?.timeoutErrorMessage || 'Timeout: aucune réponse reçue pour BRIGHTNESS_GET',
    });
