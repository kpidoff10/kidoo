...
bool ModelBasicPubNubRoutes::processMessage(const JsonObject& json) {
  // Vérifier que l'action est présente (nouvelle syntaxe ArduinoJson)
...
bool ModelBasicPubNubRoutes::handleBrightness(const JsonObject& json) {
  // Format: { "action": "brightness", "value": 0-100 }
...
bool ModelBasicPubNubRoutes::handleSleep(const JsonObject& json) {
  // Format: { "action": "sleep", "timeout": 30000 } (en ms, 0 = désactivé)
  // Ou: { "action": "sleep", "enabled": false }
  
  if (json["enabled"].is<bool>()) {
    bool enabled = json["enabled"].as<bool>();
    if (!enabled) {
      // Désactiver le sleep mode
      SDConfig config = SDManager::getConfig();
  ...
  Serial.println("[PUBNUB-ROUTE] sleep: parametre 'timeout' ou 'enabled' manquant");
  return false;
}
...
bool ModelBasicPubNubRoutes::handleStatus(const JsonObject& json) {
  // Format: { "action": "status" }
  // Publie le statut complet de l'appareil
  
  SDConfig config = SDManager::getConfig();
  
  char statusJson[256];
  snprintf(statusJson, sizeof(statusJson),
    "{\"type\":\"status\",\"device\":\"%s\",\"ip\":\"%s\",\"brightness\":%d,\"sleep_timeout\":%lu,\"wifi_rssi\":%d}",
    DEFAULT_DEVICE_NAME,
  ...
  Serial.println("[PUBNUB-ROUTE] Statut publie");
...
void ModelBasicPubNubRoutes::printRoutes() {
  ...
  Serial.println("{ \"action\": \"sleep\", \"enabled\": false }");
